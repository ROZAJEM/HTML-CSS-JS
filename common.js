...
window.loadding = function(message){
	...
	art.dialog({"id":"loadding","lock":true,"fixed":true,"drag":false}).content(message);}
...
function getArrUrl(str, split1, split2) {
	split1 = split1 || "&";
	split2 = split2 || "=";
	var strArr = str.split(split1);
	var NewArr = [];
	for (var i = 0; i < strArr.length; i++) {
		var strArrArr = strArr[i].split(split2);
		NewArr[strArrArr[0]] = strArrArr[1];
	}
	return NewArr;
}
//延迟执行
...
//		}
//		else
//		if (event.button == 2 || event.button == 3 || e.which == 1)
//		{
//			event.cancelBubble = true
//			event.returnValue = false;
//			return false;
